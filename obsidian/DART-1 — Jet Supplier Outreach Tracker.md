...
- [ ] **Rotorsan (RTJ-900 PRO)** — ✅ **NDA executed 07-27**; reminder sent 08-04 → **Bülent's initial technical response received 08-04** (12-section, RTJ-900 PRO confirmed as the proposed engine). **Ball now MINE** (review + await first release + call windows). **Front-runner.**
    ...
    - **✅ NDA sharing gate CLEARED (08-04):** Executed mutual NDA (DocuSign complete; mutual edits present; term 3 yr + 5 yr survival; Bursa/Türkiye law). JJ signed as "Individual" → §5 required **Rotorsan's prior written consent** to share with team. **Bülent's written consent received 08-04** naming Michael Mente (sponsor) & Kenny "Tino" Salim (team) as permitted recipients. Team may now receive Rotorsan Confidential Information on a need-to-know basis.
    - **📁 Confidential folder** `DART-1 / Rotorsan — Confidential (NDA)` — **shared 08-04 with Michael & Tino**. Contains: (1) 08-04 RTJ-900 PRO informational email (Google Doc), (2) team Confidentiality Acknowledgment (Michael + Tino to sign), (3) executed NDA PDF (drag in if not yet added). Keep all Rotorsan data here — not on public channels or the git repo.
