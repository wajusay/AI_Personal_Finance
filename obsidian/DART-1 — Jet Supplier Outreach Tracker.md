...
> **Last refreshed:** 2026-08-04 · **Suppliers tracked:** 19 (6 core + 13 expansion) · **Ball in my court:** 3 · **Ball with supplier:** 13 · **Declined:** 3
> *08-04: 🚀 **Rotorsan sent the initial RTJ-900 PRO technical response** (12-section) — altitude/thrust/two-flight all fit; **mass/packaging is the open gate** (10.2 kg engine-only vs 5.9 kg bay allocation). Prioritized first document release + call windows to follow. Ball mine (review). Front-runner well ahead.*
> *07-29 sweep: ❌ **ZofiTech DECLINED 07-29 02:40 PDT** — "not able to serve/deliver to individual customers; focused on institutional OEMs" (a firm OEM-only policy decline, distinct from the earlier "manned" mix-up; unmanned clarification did not reopen it). Closed. · ✅ **Rotorsan mutual NDA FULLY EXECUTED via DocuSign 07-27** — sent 07-29 confirming NDA + requesting RTJ-900 PRO data; 7/31 call now off (my flight); short follow-up sent 07-29 to stand down 7/31 + request next availability. Ball with Rotorsan (data + a time). **Frank Turbine** follow-up sent, no reply yet (ball theirs). In my court: KingTech, DG Propulsion.*
...
- [ ] **Rotorsan (RTJ-900 PRO)** — ✅ **NDA executed 07-27**; reminder sent 08-04 → **Bülent's initial technical response received 08-04** (12-section, RTJ-900 PRO confirmed as the proposed engine). **Ball now MINE** (review + await first release + call windows). **Front-runner.**
    - **Published/confirmed values:** thrust 110–900 N (idle 110 N @30k rpm → max 900 N @61k rpm); mass flow ~1.8 kg/s; PR ~4.2; SFC ~0.156 kg/N·h; fuel 360–2,820 mL/min (kerosene); max EGT 750 °C; **28 VDC**, starter-gen up to **1 kW**, ~5 s start; MIL-STD-810/461 basis; **engine-only mass 10.2 kg**, 496 mm × Ø203 mm; **max operating altitude 10,000 m (~32,800 ft)**; **max STARTING altitude 5,000 m**.
    - **Gate read:** ✅ altitude 10,000 m covers 30k ft; ✅ 110–900 N brackets ~550 N target w/ reserve; ✅ two-flight/day profile explicitly recorded (cool-down/inspection TBC); ⚠️ 5,000 m start ceiling = **ground restart only** (fine between flights; issue only if in-flight relight >16,400 ft needed); 🔴 **MASS/PACKAGING GATE** — 10.2 kg is engine-only (installed likely ~13–15 kg) vs 25 kg MTOW & ~190×450 mm / 5.9 kg bay allocation; 203 mm dia & 496 mm length exceed the bay. **Open risk on the front-runner — installed-mass sheet will settle it.**
    - **Coming (their prioritized first release):** supply config; installed-mass breakdown + CG + bolt pattern; prelim operating limits; 30k-ft / 61 kPa / 282 K design-point assessment (marked measured/modeled/extrapolated); fuel + electrical interface summaries; initial quote. Full ICD (pinouts, throttle/telemetry protocol, fault codes, shutdown architecture), duty/life/TBO, and export docs to follow. Call windows to be sent. *(Note: they flag old engine-family figures — DB9/RS232, filtration/pressure — do NOT carry to the PRO until revalidated.)*
    - **⚠️ NDA sharing gate:** Executed mutual NDA reviewed 08-04 (DocuSign complete; mutual edits present; term 3 yr + 5 yr survival; Bursa/Türkiye law). **Signed by JJ as "Individual"** → §5 permits disclosure only to "employees and professional advisers" on need-to-know; Michael (sponsor) & Tino (team) don't cleanly fit and count as third parties, so sharing Rotorsan's data with them needs **Rotorsan's prior written consent** (§5 bullet 1). **Consent request drafted 08-04** to Bülent naming both recipients (bound by equal confidentiality). Send before sharing engine data with the team; also have Michael/Tino acknowledge confidentiality internally.
    - **📁 Confidential folder staged 08-04** (My Drive, **owner-only / NOT shared**): `DART-1 / Rotorsan — Confidential (NDA)` — contains (1) the 08-04 RTJ-900 PRO informational email as a Google Doc, and (2) the team Confidentiality Acknowledgment (Michael + Tino signature blocks). **To add:** drag the executed NDA PDF into the folder (too large to upload via tool). **Do not share** until Bülent's consent lands; then add Michael & Tino as viewers.
...
| **Frank Turbine** | FT400 / FT500 | Austria | Sandro Bombek (s.bombek@frankturbine.com; cc office@) | 2026-07-28 | **Theirs** | 📄 **Full answers 07-27:** installed ~4.5 kg (workable), two-flight/restart OK, **exportable to US private buyer**, standard nozzle only. ⚠️ Untested at 9 km — **no altitude data** (validation on us); 440 N < ~550 N target. **Follow-up sent 07-28** for remaining engine-agnostic interface/limits data (see Next actions). **Viable fallback (#2)** behind Rotorsan |
...
| **Rotorsan** | RTJ-900 / RTJ-900 PRO | Türkiye | Bülent Akyürek (info@rotorsan.com) | 2026-08-04 | **Mine** | ✅ NDA executed 07-27. **Initial 12-section technical response 08-04** (RTJ-900 PRO confirmed): 110–900 N, 30k–61k rpm, ~1.8 kg/s, PR 4.2, SFC 0.156, 750 °C, 28 VDC/1 kW/5 s start, kerosene, 10.2 kg engine-only, 496×Ø203 mm, op alt 10,000 m, start alt 5,000 m. ✅ altitude/thrust/two-flight fit; ⚠️ start-alt 5,000 m (ground restart only); 🔴 mass/packaging gate. Prioritized first release + call windows coming. **Front-runner** |
